...
# the R0 that are going to be used
R0_scales = [0.1, 0.5, 0.75, 1, 1.25, 1.5, 2]
...
for R0_scale in R0_scales:
    filename = "output_R0scale_%.1E.dat" % R0_scale
    command = "cd ../build && ./main.x %g %g %g %g %g %s &> /dev/null" % (R0_scale,M,rho,T,dm,filename)
    print "\nRunning with args: %g %g %g %g %g %s" % (R0_scale,M,rho,T,dm,filename)
    os.system(command)
...
for i, data in enumerate(allData):
    ...
    mfig.plot(r, m,
            label=(r'$R_0/R_{0,i}=%s$' % R0_scales[i]))
    Pfig.semilogy(r, P,
            label=(r'$R_0/R_{0,i}=%s$' % R0_scales[i]))
    Lfig.plot(r, L,
              label=(r'$R_0/R_{0,i}=%s$' % R0_scales[i]))
...
mfig.set_title(r'$m(r)$ for various radii')
mfig.set_xlabel(r'$r/R_0$')
mfig.set_ylabel(r'$m/M_0$')

Pfig.set_title(r'$P(r)$ for various radii')
Pfig.set_xlabel(r'$r/R_0$')
Pfig.set_ylabel(r'$P/P_0$')

Lfig.set_title(r'$L(r)$ for various radii')
Lfig.set_xlabel(r'$r/R_0$')
Lfig.set_ylabel(r'$L/L_0$')

Tfig.set_title(r'$T(r)$ for various radii')
Tfig.set_xlabel(r'$r/R_0$')
Tfig.set_ylabel(r'$T/T_0$')

rhofig.set_title(r'$\rho(r)$ for various radii')
