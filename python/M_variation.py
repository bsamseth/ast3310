...
# the M0 that are going to be used
M_scales = [0.1, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 2]
...
for M_scale in M_scales:
    filename = "output_Mscale_%.1E.dat" % M_scale
    command = "cd ../build && ./main.x %g %g %g %g %g %s &> /dev/null" % (R,M_scale,rho,T,dm,filename)
    print "\nRunning with args: %g %g %g %g %g %s" % (R,M_scale,rho,T,dm,filename)
    os.system(command)
...
for i, data in enumerate(allData):
    ...
    mfig.plot(r, m,
            label=(r'$M_0/M_{0,i}=%s$' % M_scales[i]))
    Pfig.semilogy(r, P,
            label=(r'$M_0/M_{0,i}=%s$' % M_scales[i]))
    Lfig.plot(r, L,
              label=(r'$M_0/M_{0,i}=%s$' % M_scales[i]))
