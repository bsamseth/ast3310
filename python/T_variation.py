...
# the T0 that are going to be used
T0_scales = [0.5, 0.75, 1, 1.25, 1.5, 2]
...
for T0_scale in T0_scales:
    filename = "output_T0scale_%.1E.dat" % T0_scale
    command = "cd ../build && ./main.x %g %g %g %g %g %s &> /dev/null" % (R,M,rho,T0_scale,dm,filename)
    print "\nRunning with args: %g %g %g %g %g %s" % (R,M,rho,T0_scale,dm,filename)
    os.system(command)
...
for i, data in enumerate(allData):
    ...
    mfig.plot(r, m,
            label=(r'$T_0/T_{0,i}=%s$' % T0_scales[i]))
    Pfig.semilogy(r, P,
            label=(r'$T_0/T_{0,i}=%s$' % T0_scales[i]))
    Lfig.plot(r, L,
              label=(r'$T_0/T_{0,i}=%s$' % T0_scales[i]))
